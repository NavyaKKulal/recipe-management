...
// Handle form submission for Add Recipe
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    switch ($action) {
        case 'create':
            $sql = "INSERT INTO RECIPE (Recipe_id, user_id, recipe_name, recipe_type, quantity, description) 
                    VALUES (?, ?, ?, ?, ?, ?)";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ssssss", $recipe_id, $user_id, $recipe_name, $recipe_type, $quantity, $description);
            if ($stmt->execute()) {
                ...
                $message = "Error adding recipe: " . $conn->error;
                echo "<script>alert('$message');</script>";
            }
            break;

        case 'update':
            $sql = "UPDATE RECIPE SET user_id = ?, recipe_name = ?, recipe_type = ?, quantity = ?, description = ? 
                    WHERE Recipe_id = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ssssss", $user_id, $recipe_name, $recipe_type, $quantity, $description, $recipe_id);
